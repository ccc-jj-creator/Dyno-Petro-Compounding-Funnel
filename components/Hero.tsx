...
const FIXED_RATE = 15; // Fixed annual rate
...
const Hero: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const rateDecimal = FIXED_RATE / 100;
    ...
    const chartData = useMemo(() => {
        ...
        return result.yearlyBreakdown.map(item => ({
            ...
            'Projected Growth': item.totalInterest,
        }));
    }, [result, investment]);
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="rounded-lg bg-brand-dark-blue/80 backdrop-blur-sm p-4 shadow-2xl ring-1 ring-white/10">
                    ...
                    <p style={{ color: payload[1]?.color || '#D4AF37' }}>{`Projected Growth : ${formatCurrency(payload[1]?.value)}`}</p>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="max-w-2xl">
              ...
              <p className="mt-6 text-lg leading-8 text-brand-gray">
                Use the interactive calculator to see how our proven, low-risk acquisition model can help you achieve your financial goals, with projected returns of up to 15% APY.
              </p>
              <div className="mt-10 grid grid-cols-1 gap-x-6 gap-y-4 text-base font-semibold leading-7 sm:grid-cols-2">
                <ValueProp>Up to 15% APY</ValueProp>
            ...
            <div className="w-full rounded-lg bg-brand-dark-blue/80 backdrop-blur-sm p-8 shadow-2xl ring-1 ring-white/10">
                ...
                <div className="mt-6 border-t border-white/10 pt-6">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 rounded-lg bg-brand-blue/50 p-4">
                        <div className="text-center">
                            <p className="text-xs sm:text-sm text-brand-gray">Projected Monthly Return</p>
                            <p className="text-lg sm:text-xl font-bold text-white">{formatCurrency(result?.monthlyInterest ?? 0)}</p>
                        </div>
                        <div className="text-center">
                        ...
                        <div className="text-center">
                            <p className="text-xs sm:text-sm text-brand-gray">Total Profit</p>
                            <p className="text-lg sm:text-xl font-bold text-white">{formatCurrency(result?.totalInterest ?? 0)}</p>
                        </div>
                    </div>
                </div>
            </div>
                     ...
                     <div className="h-96 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                                ...
                                <Area type="monotone" dataKey="Projected Growth" stackId="1" stroke="#D4AF37" strokeWidth={2} fill="url(#colorInterest)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                 </div>
